...
void create()
{
	set_short("@@query_c_name");    
        ...
        set("c_realname","付子菌");         // 玩家辨认成功传回的名字
   	...
   	set("value", ({0, "silver"}) );       // 野生的树当然不值钱啊
        ...
        set("herb_file","ascomycete.c") ;     //提炼出来的东东的完整档名
        set("herb_path","/d/healer/reagent/");   //提炼出来的东东的目录
                                              //记得用完整目录
                                              //default 是 /d/healer/plant
                                                
        set("skill_require",50) ;             //要成功辨识这种植物所须之技能
...
string query_c_name()
{
  ...
  if (this_player()->query_stat("int") < 10 ) {
    ...
    set_name("ascomycete","沼泽苔");
    set("id",({"plant","tree","ascomycete"}) );
    set_long("根据你仔细鉴定的结果, 你判断这棵孢子植物上长了些付子菌(ascomycete),\n"
