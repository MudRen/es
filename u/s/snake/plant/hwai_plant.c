...
string query_c_name()
{
  ...
  if (this_player()->query_stat("int") < 10 ) {
    ...
    set_long("根据你仔细鉴定的结果, 你判断这是一棵槐树(hwai),\n"
             "如果你有药锄, 还可以挖(dig)一点回去。\n");
    set_name("chuan chiung","槐树");          
    return "槐树";
  }  
  else {
    set("id",({"plant","tree"}) );
