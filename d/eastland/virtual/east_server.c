...
object compile_object(string s)
{
	...
	if( y>0 ) {
		if( undefinedp( legends[map[y-1][x]]["block_move"] ) )
			obj->add("exits", (["north": (string)fname + "/" + x + "," + (y-1) + ".east" ]) );
	}
	if( y<height-1 ) {
		if( undefinedp( legends[map[y+1][x]]["block_move"] ) )
			obj->add("exits", (["south": (string)fname + "/" + x + "," + (y+1) + ".east" ]) );
	}
	if( x>0 ) {
		if( undefinedp( legends[map[y][x-1]]["block_move"] ) )
			obj->add("exits", (["west" : (string)fname + "/" + (x-1) + "," + y + ".east" ]) );
	}
	if( x<width-1 ) {
		if( undefinedp( legends[map[y][x+1]]["block_move"] ) )
			obj->add("exits", (["east" : (string)fname + "/" + (x+1) + "," + y + ".east" ]) );
	}
	obj->reset();
	return obj;
}
