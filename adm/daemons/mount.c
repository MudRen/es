...
int mount(object me,object who)
{
	...
	// This is a wild beast.
		if( master_skill/2 + random(master_skill/2) > (int)me->query_level() * 5 ) {
			me->set("mountable", 1);
			tell_object(who,sprintf("你跳到%s的背上，并且成功地将它驯服了！\n",me->query("c_name") ));
			tell_room( environment(me), 
				sprintf("%s跳到%s的背上，并且成功地驯服了它！\n" ,
				who->query("c_name"),me->query("c_name") ), who );
			me->cease_all_attacks();
		} else {
			tell_object(who,sprintf("你企图跳到%s背上，但是很快地摔了下来！\n",me->query("c_name")));
			tell_room( environment(me), 
				sprintf("%s跳到%s的背上，但是很快地被摔了下来！\n",
				who->query("c_name"),me->query("c_name")), who );
			me->kill_ob(who);
			who->block_attack(8);
			return 1;
	...
	tell_object(who,sprintf("你跳到%s 背上，如果要下来，用 dismount　\n",me->query("c_name")));
...
int dismount(object me,object who)
{
	...
	tell_object(who,sprintf("你从%s的背上跳了下来.　\n",mount_ob->query("c_name") ));
	tell_room( environment(who), 
		sprintf("%s从%s 的背上跳了下来.　\n" ,
