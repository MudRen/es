...
log_file("ftp","query_connections ok\n");
    return i ? list[0..i-1] : ({ });
}
...
log_file("ftp","begin in_write_callback\n");
...
log_file("ftp","in_write_callback ( begin to socket_write ) \n");
  res = socket_write( fd,
...
log_file("ftp","in_write_callback ( socket_write finished) \n");
  /*
...
void in_listen_callback( int fd ) {
    int new_fd;
log_file("ftp","begin in_listen_callback\n");

    if ( ( new_fd =
socket_accept( fd, "in_read_callback", "in_write_callback" ) ) < 0 )
{   log_file("ftp","socket_accept Fail \n"); return; }

    number_of_users++;
    socket_info[new_fd] = ([  ]);
    socket_info[new_fd][LAST_DATA] = time();
log_file("ftp","in_listen_callback ok \n");
...
log_file("ftp","begin in_read_callback\n");
    if (undefinedp( socket_info[ fd ] ) || !socket_info[ fd ][ CONNECTED ])
{ log_file("ftp","undefinedp socket_info \n"); return; }
...
void in_close_callback( int fd ) {
     ...
     */
log_file("ftp","begin in_close_callback\n");
    if (undefinedp(socket_info[fd][CONNECTED]))
  ;
    else {
  /*
