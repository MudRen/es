...
build_mudlist(mapping muds)
{
  ...
  for (i=0,pos=0;i<sizeof(names);i++)
    {
//tell_object(find_player("grendel"),sprintf("doing: %s\n", names[i]));
      ...
      ret[pos] += "||"+i+":"+
		"|NAME:" + muds[names[i]]["NAME"] +
	     ...
	     (undefinedp(muds[names[i]]["OFFSET"]) ? "" 
					       : "|OFFSET:"+muds[names[i]]["OFFSET"]);
...
void
send_mudlist_q(string host, string port)
{
