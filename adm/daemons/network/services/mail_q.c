...
inherit DAEMON;
// This might be /std/save.c on your mud.
inherit "/std/save";
...
varargs
void check_for_mail( string mudname, int flag ) {
  mapping	outmsg, info;
  string	package;
  int	index, size;
...
//  info["MSG"] = strip_backspace( info["MSG"] );
  ...
  if( info["ENDMSG"] ) {
    ...
    field = "Received mail to: " + implode( info["WIZTO"], ", " ) + ".";
  
    catch( "/adm/daemons/network/services/affirmation_a" -> send_affirmation_a( 
            info["HOSTADDRESS"], info["PORTUDP"],
		"POSTMASTER",
            info["WIZFROM"], field, "mail_q" ) );
