...
#include <uid.h>
 ...
 static int *level_exp, *skill_exp, *stat_exp;

 static string *stat_names = ({ "strength", "intelligence",
	"dexterity", "constitution", "piety", "karma", });

 static string *special_defense_names = ({
...
string alignment_string(int align)
{
	...
	else if( align < 200 ) return set_color("无功\无过", "HIW");
	...
	else if( align < 5000 ) return set_color("功\德无量", "HIC");
...
string body_status_string(object obj)
{
	...
	else if( percent < 60 ) return set_color("你觉得头重脚轻、全身乏力，也许\是药吃太多了。", "HIG", obj);
...
varargs string status_string(object obj)
{
	...
	else if( percent < 60 ) return sprintf("伤到不少地方，%s。", set_color("流了许\多鲜血", "HIY", obj));
	...
	else if( percent < 60 ) return "伤到不少地方，流了许\多鲜血。" ;
