...
int do_cost(string arg)
{
        ...
        if( lvl >= MAX_ORG_LEVEL ) msg ="你已经升到最高级了。\n";
        else if( exp > 0 ) msg ="你还需要 "+exp+" 点经验才能升级。\n";
        else msg = "你的经验值已经达到升级标准了。\n";
                
        stock = query_exp_train(me);
        if( stock < 1 ) 
                msg +="你目前没有任何额外的经验可以用来提升技能。\n";
        else 
	{
              msg +="并且你有 "+stock+" 点经验可以用来提升技能。\n";
...
int do_train(string arg)
{
        ...
        me = this_player();
        if( !this_guild->is_guild_member(me) )
                return notify_fail("很抱歉，你不是这个组织的成员。\n");
...
int do_join(string arg)
{
        ...
        if( wizardp(ob) ) {
	        ...
	        me=ob;   
                exp =  query_org_exp(0);
                if( exp > (int)me->query_experience() )
                        return notify_fail("你的经验值太低。\n");

                // check quest point, added by Kyoko.
                if( query_org_quest(me)>me->query_quest_points())
                        return notify_fail("你的任务点数太低。\n");
...
int do_explore(string arg)
{
        ...
        if( expl > 1000 ) expl = 1000;
        printf(set_color("平均探险度 : %2d.%1-d%%    升级标准：%d%%.\n","HIC"), 
...
指令格式: advance level 
    ...
    当你的经验值足够时，这个指令能提升你的等级。可是，如果
你的探险度及任务点数未达标准时，即使经验值足够亦不能升级。
USAGE_ADVANCE
                        ); break;
